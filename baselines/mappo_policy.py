"""MAPPO custom policy: centralized critic using the global observation.

Actor uses the local observation; the critic uses the concatenated global
observation. Ported from the sibling IIGC repo
(``AAAI2027-510k-clear/src/env/mappo_policy.py``).
"""
...
class MAPPOCentralizedCriticPolicy(MaskableActorCriticPolicy):
    ...
    def _build_mlp_extractor(self):
        self.mlp_extractor = MAPPOMlpExtractor(actor_dim=256, critic_dim=512)
...
class MAPPOFeatureExtractor(BaseFeaturesExtractor):
    ...
    def forward(self, obs):
        return self.local_net(obs["local"]), self.global_net(obs["global"])


class MAPPOMlpExtractor(nn.Module):
    """Actor processes local features; critic processes global features."""
