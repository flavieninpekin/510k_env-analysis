...
and measure it behaviourally: policies learn to defer to a fixed, known partner
(deferral asymmetry $+0.38$, $p<10^{-4}$), but do not condition on a hidden
partner ($+0.00$, $p=0.21$) and do not exploit one that is explicitly revealed
($-0.04$, $p<10^{-4}$), even though the reveal dial is a near-lossless
...
masked-PPO suite and a recurrent variant corroborate the failure. 510K is
