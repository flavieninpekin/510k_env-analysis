...
\paragraph{Recurrent corroboration.} The MLP is memoryless, so we also trained
two masked-LSTM variants (1M steps, 2 seeds). The healthiest is seed-diverse in
...
(DYNAMIC $-0.043$; OBVIOUS $-0.07/-0.06$), so the failure is not merely
memoryless; its DYNAMIC checkpoints remain seed-degenerate (corroborating
evidence only).
