...
\title{510K: A Card-Game Testbed for Dynamic Cooperation, Partial Observability,
and Information Revelation in Multi-Agent RL}
...
partial observability. We present \textbf{510K}, a four-player shedding card
game that \emph{couples} these challenges in a single decision process: the axes
are not merely stacked but alter one another's value, and the cooperative
relationship itself is a hidden latent variable --- team membership --- and even
team size (2v2 vs 1v3) --- is determined at deal time by a private red-A
distribution and must be inferred from observed play. 510K ships with four
controllable modes (SINGLE, STATIC, DYNAMIC, OBVIOUS) that ablate the
...
characterize the coupling with three measurements: (i) structural probes show
the immediate-score and terminal-finish objectives are in genuine tension, and
that the hidden team leaves a large non-attributable share of reward variance;
(ii) a 1M-step, 5-seed masked-PPO suite shows SINGLE is hardest for every method
(win rate $\approx$ chance), while team-mode win rates remain inflated by
teammate and team-selection effects; (iii) a behavioural probe finds policies
learn to cooperate with a \emph{fixed, known} partner (deferral asymmetry
$+0.38$) but fail to infer a \emph{hidden} partner ($+0.00$, null) and do not
exploit one that is \emph{explicitly revealed} ($-0.04$, with no performance
gain) --- a direct echo of IIGC's \emph{deceptive stability}. 510K is positioned
as a testbed in which individually-studied challenges are jointly controlled and
ablatable.
\end{abstract}
...
closed-form hypergeometric fact verified exactly (Section~6.8). We treat the
...
\paragraph{Recurrent corroboration.} A natural objection is that the MLP is
memoryless and \emph{cannot} infer the hidden relationship. We therefore
trained two masked-LSTM variants (higher entropy; smaller net + lower lr) for
1M steps, 2 seeds. The healthiest (\texttt{ent\_coef}=0.05) is seed-diverse in
SINGLE ($0.540 \pm 0.030$) and STATIC and learns fixed-partner cooperation
($+0.37/+0.38$), yet it still shows no cooperation under hidden or revealed
teams (DYNAMIC $-0.043$, seed-identical; OBVIOUS $-0.07/-0.06$). The
cooperation-inference failure is therefore not simply an artifact of a
memoryless policy. Caveat: the DYNAMIC checkpoints remain seed-degenerate, so
we treat this as corroborating rather than definitive evidence.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.58\linewidth]{figs/learning_curves_modes.png}
...
plus coupling / positions / team-conditioning analyses and the statistical
report (\texttt{runs/stats\_report.md}).
\end{itemize}
